...
import { EActionTypes } from "../../../app/store/basket/types";
import { useTypedSelector } from "../../../app/store/hooks/hooks";
import { addData, getData, getProducts } from "../../../app/store/basket/actions";

import { createMassiveFromNumber } from "../../../shared/scripts/arrays/arrays";
import { REPEAT_LOAD, UPDATE_NUMBER_PAGE } from "../../../app/store/basket/basketReducer";
import { PRODUCTS } from "../../../entities/const/const";

export const Catalog: React.FC = () => {
    if (!sessionStorage.getItem("pageNumber")) sessionStorage.setItem("pageNumber", JSON.stringify(1));
    const [numberActualPage, setNumberActualPage] = useState<number>(JSON.parse(sessionStorage.getItem("pageNumber") as string));
    const presence = useTypedSelector(state=>state.presence.presence);
    ...
    return (
        <div className="bodyCatalog">
            ...
            <Button onClick={()=>{
                ...
                sessionStorage.setItem("pageNumber", `${numberActualPage + 1}`);
            ...
            <div className="pagination">
                ...
                </span>
                {
                    massive.map((el:number)=>{
                        return (
                            <span className={`${numberActualPage == el ? "actualPage" : ""} ${(` ${(numberActualPage-1 ? numberActualPage-1 : 3)} ${numberActualPage} ${numberActualPage==countPages ? countPages-2 : ""} ${(numberActualPage+1)} `).includes(el.toString()) ? "" : "outSideGroup"}`} onClick={()=>{
                                if (el==pageNumber) dispatch(REPEAT_LOAD());
                                dispatch(UPDATE_NUMBER_PAGE(el));
                                sessionStorage.setItem("pageNumber", JSON.parse((`${el}`)));
                ...
                <span onClick={()=>{
                    ...
                    sessionStorage.setItem("pageNumber", JSON.parse((`${countPages}`)));
